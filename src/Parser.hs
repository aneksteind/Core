...
happyReduction_3 ((HappyAbsSyn7  happy_var_4) `HappyStk`
	...
	(HappyTerminal (T TokenSym p happy_var_1)) `HappyStk`
	happyRest)
	 = HappyAbsSyn5
...
happyReduction_5 (HappyAbsSyn6  happy_var_2)
	(HappyTerminal (T TokenSym p happy_var_1))
	 =  HappyAbsSyn6
...
happyReduction_22 ((HappyAbsSyn7  happy_var_5) `HappyStk`
	...
	(HappyTerminal (T TokenSym p happy_var_2)) `HappyStk`
	_ `HappyStk`
	happyRest)
	 = HappyAbsSyn7
...
happyReduction_24 (HappyTerminal (T TokenSym p happy_var_1))
	 =  HappyAbsSyn8
		 (EVar happy_var_1
	)
...
happyReduction_25 (HappyTerminal (T (TokenInt) p happy_var_1))
	 =  HappyAbsSyn8
		 (ENum (read happy_var_1 :: Int)
	)
...
happyReduction_26 (_ `HappyStk`
	...
	_ `HappyStk`
	_ `HappyStk`
	(HappyTerminal (T (TokenInt) p happy_var_5)) `HappyStk`
	_ `HappyStk`
	(HappyTerminal (T (TokenInt) p happy_var_3)) `HappyStk`
	_ `HappyStk`
	_ `HappyStk`
	happyRest)
	 = HappyAbsSyn8
...
happyReduction_37 ((HappyAbsSyn7  happy_var_6) `HappyStk`
	...
	_ `HappyStk`
	(HappyTerminal (T (TokenInt) p happy_var_2)) `HappyStk`
	_ `HappyStk`
	happyRest)
	 = HappyAbsSyn14
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	T (TokenInt) p happy_dollar_dollar -> cont 15;
	T TokenSym p happy_dollar_dollar -> cont 16;
	T TokenAdd p _ -> cont 17;
	T TokenMin p _ -> cont 18;
	T TokenMul p _ -> cont 19;
	T TokenDiv p _ -> cont 20;
	T TokenAssign p _ -> cont 21;
	T TokenLamVars p _ -> cont 22;
	T TokenLamExpr p _ -> cont 23;
	T TokenLT p _ -> cont 24;
	T TokenLTE p _ -> cont 25;
	T TokenEQ p _ -> cont 26;
	T TokenNEQ p _ -> cont 27;
	T TokenGTE p _ -> cont 28;
	T TokenGT p _ -> cont 29;
	T TokenAnd p _ -> cont 30;
	T TokenOr p _ -> cont 31;
	T TokenLet p _ -> cont 32;
	T TokenLetRec p _ -> cont 33;
	T TokenIn p _ -> cont 34;
	T TokenCase p _ -> cont 35;
	T TokenOf p _ -> cont 36;
	T TokenArrow p _ -> cont 37;
	T TokenPack p _ -> cont 38;
	T TokenLBrace p _ -> cont 39;
	T TokenRBrace p _ -> cont 40;
	T TokenLParen p _ -> cont 41;
	T TokenRParen p _ -> cont 42;
	T TokenSemiColon p _ -> cont 43;
	T TokenComma p _ -> cont 44;
	_ -> happyError' (tk:tks)
	}
...
parseError ts =
  case ts of
    ...
    (T t p s):_ ->
      error "parse error at" ++ showPos pos ++ " - unexpected " ++ show t
